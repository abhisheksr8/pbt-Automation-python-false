...
def pipeline(spark: SparkSession) -> None:
    ...
    df_print_and_return = print_and_return(spark, df_customers_reformatted)
...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/AutomationPBT-falsepython")
    registerUDFs(spark)
    
    MetricsCollector.instrument(spark = spark, pipelineId = "pipelines/AutomationPBT-falsepython", config = Config)(
        pipeline
    )
